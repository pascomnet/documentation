...
description: After you have familiarized yourself with the basics of pascom Analytics you can learn here how you can make your own adjustments.
weight: 8
---
...
{{% notice info%}}
You can only customize database queries. The data sources for **Live Dashboards** are generated by pascom **statically**
{{% / notice%}}

With pascom Analytics you can not only create your own dashboard with existing graphs but also customize the underlying data sources.

**Requirement** for this is that you are familiar with the **basics of pascom Analytics** and **SQL**.

## Getting Started

The easiest way to understand the SQL queries used is to edit existing graphs. First copy an existing dashboard (see [Create Edit Custom Dashboards]({{< ref "/modules/statistics#create-edit-custom-dashboards" >}})). Then you can edit graphs and tables and adjust the SQl.

Click on any graph or tablet on {{< ui-button "Edit" >}}:

![Edit graph](graph_edit.png "Edit graph")

Now you can adapt and test the SQL:

![Graph Details](graph_details.png "Graph Details")

You can find further information, in particular on variables, in the editor under {{< ui-button "Show Help" >}}.

The database used is PostgreSQL. Information about PostgreSQL in connection with Grafana (Analytics Tool used by pascom) can be found here [Using PostgreSQL in Grafana](https://grafana.com/docs/grafana/latest/features/datasources/postgres/).
...
This function returns **all completed** calls. Calls that are still active at the moment of the query (live) are not
considered.
...
| ------------------ | --------------------------------- |
| from_timestamp     | Calls since that time. Usually dasboard variable ***$__timeFrom()*** for the selected period. |
| to_timestamp       | Calls up to this point. Usually dashboard variable ***$__timeTo()*** for the selected period. |
...
| status             | Possible values: ***hangup*** = call was hung up normally, ***transfer*** = call was connected, ***noanswer*** = call was not answered |
| direction          | Possible values: ***inbound*** = incoming call, ***outbound*** = outgoing call, ***internal*** = internal call |
| total_duration     | Total duration of the call. The call starts as soon as the user or team is called. IVR, call router, etc. are not counted. |
| ringing_duration   | Duration until the call is answered by a user |
...
This function returns all **completed**, **incoming** calls made via a **team**. Calls that are still active at the moment of the query (live) are not
considered.
...
In **mdphonecallrecord** every single step that a call takes through the telephone system is saved. A call therefore creates many entries
in the mdphonecallrecord. The individual entries are linked to a call via the **phonecallrecord_chain** field.

Please mainly use the SQL functions offered by pascom and only access this table for any details if needed.
...
As a step, we understand every object in pascom that can be reached via an extension.

In our example, the call flow is as follows:

- Caller calls user A directly (101)
- User A puts user A on hold and calls user B (102)
- User A connects caller to user B (102)

The sample call flow therefore creates free entries in the database (only excerpts):
...
- Caller is asked for the team in IVR menu and chooses 1 (501)
...
- The total duration of the call is therefore the duration of step 1 and step 2, ie 37 seconds.
...
| phonecallrecord_chain | The steps to a call are linked using this ID |
...
| phonecallrecord_srcprefix | Caller prefix. Only for incoming callers. Prefix of the office through which the call came |
...
| phonecallrecord_holdcount | How many times was the call on hold in this step |
| phonecallrecord_holdduration | How long was the call held in this step? |
